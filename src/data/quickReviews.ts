...
export async function getPaginatedReviews(page: number, rating?: string, options: { postsPerPage?: number } = { postsPerPage: 20 }) {
...
export async function getRecentReviews(limit: number = 4): Promise<QuickReview[]> {
  const { reviews } = await getPaginatedReviews(1, undefined, { postsPerPage: limit });
  return reviews;
}